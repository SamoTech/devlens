...
export default async function RepoPage({ params }: Props) {
  ...
  const dims: { label: string; key: string; weight: string }[] = [
    { label: "README Quality",   key: "readme",    weight: "20%" },
    { label: "Commit Activity",  key: "commits",   weight: "20%" },
    { label: "Repo Freshness",   key: "freshness", weight: "15%" },
    { label: "Documentation",    key: "docs",      weight: "15%" },
    { label: "CI/CD Setup",      key: "cicd",      weight: "15%" },
    { label: "Issue Response",   key: "issues",    weight: "10%" },
    { label: "Community Signal", key: "community", weight: "5%" },
  ];
  ...
  return (
    <main style={{ maxWidth: "720px", margin: "0 auto", padding: "var(--space-10) var(--space-4)", display: "flex", flexDirection: "column", gap: "var(--space-8)" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "var(--space-3)" }}>
        {dims.map(({ label, key, weight }) => (
          <DimBar key={key} label={label} score={report.dimensions?.[key] ?? 0} weight={weight} />
        ))}
      </div>
